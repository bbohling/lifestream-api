...
export class KomService {
  ...
  async getKomActivities(athleteId, limit = 50) {
    try {
      ...
      const komRows = await this.prisma.kom.findMany({
        where: { athleteId: aid },
      ...
      const activitiesMeta = await this.prisma.activity.findMany({
  ...
  async getKomStats(athleteId) {
    try {
      ...
      const totalKoms = await this.prisma.kom.count({ where: { athleteId: aid, komRank: { not: null } } });

      const activityRows = await this.prisma.kom.findMany({ where: { athleteId: aid }, select: { activityId: true } });
      const activitiesWithKoms = new Set(activityRows.map((r) => r.activityId.toString())).size;

      const best = await this.prisma.kom.aggregate({ where: { athleteId: aid, komRank: { not: null } }, _min: { komRank: true } });
  ...
  async getAllKomsWithDetails(athleteId) {
    try {
      ...
      const activitiesMeta = await this.prisma.activity.findMany({ where: { id: { in: activityIds } }, select: { id: true, name: true, startDate: true } });
      const metaById = new Map(activitiesMeta.map((a) => [a.id.toString(), a]));

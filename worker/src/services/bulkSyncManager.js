...
import { BULK_SYNC, SUBREQUEST_BUDGET } from '../config.js';
...
export class BulkSyncManager {
   ...
   * @constructor
   */
  constructor(prisma, stravaService, activityService) {
    this.prisma = prisma;
  ...
  async resetBulkSync(userId) {
    try {
      ...
      await this.prisma.bulkSyncSummaries.deleteMany({
        where: { userId },
      });

      // Reset or delete existing state
      await this.prisma.bulkSyncState.deleteMany({
        where: { userId },
      });
  ...
  async getBulkSyncState(userId) {
    try {
      let state = await this.prisma.bulkSyncState.findUnique({
        where: { userId },
      });
      ...
      if (!state) {
        ...
        state = await this.prisma.bulkSyncState.create({
          data: {
            userId,
  ...
  async storeSummaries(userId, summaries) {
    try {
      ...
      for (const summary of summaries) {
        try {
          await this.prisma.bulkSyncSummaries.upsert({
            where: {
              userId_activityId: {
                userId,
  ...
  async updateBulkSyncState(userId, updates) {
    try {
      return await this.prisma.bulkSyncState.update({
        where: { userId },
        data: {
          ...updates,
          updatedAt: new Date(),
  ...
  getRemainingRequests(state) {
    const SAFETY_LIMIT = BULK_SYNC.dailyLimit ?? 950; // Strava daily read limit minus buffer
  ...
  async fetchAllSummariesPhase(userId, accessToken) {
    ...
    // Workers cap subrequests per invocation — stop before the cap and let a
    // resume call pick up from the persisted currentPage.
    while (remainingRequests > 0 && requestsUsed < SUBREQUEST_BUDGET) {
      try {
        logger.info(`Fetching summaries page ${currentPage} (${remainingRequests} requests left today)`);
        
        const summaries = await this.stravaService.fetchActivitySummaries(
          accessToken, 
          currentPage, 
          200 // Max per page
        );
    ...
    // Phase complete - update state. Only advance to detail_fetch when we
    // actually saw the last page; a budget stop resumes summary_fetch.
    const phaseComplete = sawLastPage;
    ...
    if (remainingRequests === 0) {
      ...
      logger.info(`Invocation budget reached. Call resume to continue the summary phase.`);
    } else {
  ...
  async fetchDetailsPhase(userId, accessToken, activitySummaries = null) {
    ...
    if (activitySummaries) {
      ...
      const summaryData = await this.prisma.bulkSyncSummaries.findMany({
        where: { userId },
    ...
    // Process activities in batches (also bounded by the per-invocation
    // subrequest budget — resume calls continue from persisted state)
    for (
      let i = 0;
      i < remainingIds.length && remainingRequests > 0 && requestsUsed < SUBREQUEST_BUDGET;
      i += this.batchSize
    ) {
      ...
      try {
        ...
        const detailedActivities = await this.stravaService.fetchActivityDetails(
          accessToken, 
          batchToProcess,
          this.batchSize
        );
        ...
        for (const activity of detailedActivities) {
          const transformed = this.stravaService.transformActivity(activity);
          await this.activityService.upsertActivity(transformed);
        }
  ...
  async runBulkSync(userId, accessToken) {
    ...
    await this.stravaService.fetchAndUpsertAthleteGear(accessToken, athleteId);
  ...
  async resumeBulkSync(userId, accessToken) {
    ...
    if (state.phase === 'summary_fetch' || state.phase === 'paused_daily_limit') {
      ...
      // If we hit the daily limit or the invocation budget during the summary
      // phase, return paused — a resume call continues from persisted state
      if (summaryResult.remainingRequests === 0 || !summaryResult.phaseComplete) {
...
export default BulkSyncManager;

...
import { STRAVA_API_BASE_URL, STRAVA_OAUTH_URL, RATE_LIMITS } from '../config.js';
...
export class StravaService {
   ...
   * One instance per request on Workers: rate-limit state is rebuilt from
   * Strava's response headers on the request's first API call, so per-request
   * instances track real usage just as well as the old long-lived singleton.
   * @constructor
   */
  constructor({ clientId, clientSecret, prisma }) {
    ...
    // Missing credentials only matter when a Strava call is attempted;
    // refreshToken() checks and reports them with a helpful message.
  }
